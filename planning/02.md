...
## 4. Mathematical summary of your drift model

Putting it all in one place:

* Let (d_k) = sensor drift (bias) at discrete time step (k).
* Let (x_k) = true underlying signal (unknown).
* Let (y_k) = measured observation (what sensor reports).
* Process models:
  
  $$
  x_{k+1} &= x_k + w_k^{(x)}, \quad w_k^{(x)} \sim \mathcal{N}(0, \sigma_x^2) \
  d_{k+1} &= \phi, d_k + w_k, \quad w_k \sim \mathcal{N}(0, \sigma_d^2)
  $$
* Measurement model:

  $y_k = x_k + d_k + v_k, \quad v_k \sim \mathcal{N}(0, \sigma_v^2)$

Where (\phi), (\sigma_d^2), (\sigma_x^2), (\sigma_v^2) are parameters to be set or estimated.
You can also include initial state assumptions:


$$\begin{pmatrix} x_0 \ d_0 \end{pmatrix} \sim \mathcal{N}!\bigl(\hat{x}_0,, P_0 \bigr)$$

This is a **linear Gaussian state-space model** with augmented drift state. You can use this as the basis for estimation (e.g. via Kalman filter).

If you want, you can also extend to **nonlinear drift** by letting (\phi) or the transition of (d_k) be a function (e.g. temperature-dependent), then you would use EKF / UKF.

Alternatively, if you use a **Gaussian process drift model**, your formulation is:

[
d(t) \sim \mathcal{GP}(0, K(t, t')),
\quad y_k = x_k + d(t_k) + v_k
]

Then you treat (d(t_k)) as latent GP functions and perform inference (via GP regression / Kalman-style filtering) to estimate drift over time.

---

If you like, I can now draft a full **“Stochastic Drift Model Formulation”** section (as you would write in your report) with definitions, equations, and explanation — and also a variant using GP so you can choose. Do you want me to prepare that for you now?
